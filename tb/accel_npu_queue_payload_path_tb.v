...
module accel_npu_queue_payload_path_tb;
    reg clk;
    reg resetn;
    reg enable;
    reg decode_valid;
    ...
    initial begin
        ...
        if (payload_fetch_count !== 32'h0000_0001 || payload_fetch_error_count !== 32'h0000_0000) begin
            $display("FAIL: queue payload fetch counters mismatch.");
            $finish;
        end

        if (payload_input0 !== 32'h0002_0001 || payload_input1 !== 32'h0004_0003 ||
            payload_weight0_a !== 32'h0002_0002 || payload_weight1_b !== 32'h0200_0200) begin
            $display("FAIL: queue payload words mismatch.");
            $finish;
        end

        if (payload_m_axi_arsize !== 3'b010 || payload_m_axi_arburst !== 2'b01) begin
            $display("FAIL: queue payload AXI metadata mismatch.");
            $finish;
        end

        wait (npu_start_pulse == 1'b1);
        if (npu_model_id_exec !== 8'h80 || npu_seq_length_exec !== 16'h0008) begin
            $display("FAIL: queue payload exec metadata mismatch.");
            $finish;
        end
        ...
        if (!store_request_pulse) begin
            $display("FAIL: queue payload-path did not emit store_request_pulse.");
            $finish;
        end

        if (dispatch_status !== 32'h0000_0004 || payload_fetch_status !== 32'h0000_0004 ||
            exec_status !== 32'h0001_8004) begin
            $display("FAIL: queue payload status mismatch. dispatch=%08x fetch=%08x exec=%08x",
                     dispatch_status, payload_fetch_status, exec_status);
            $finish;
        end

        if (last_input_offset !== 32'h0000_0400 || last_weight_offset !== 32'h0000_0420 ||
            last_output_offset !== 32'h0000_0480) begin
            $display("FAIL: queue payload offsets mismatch.");
            $finish;
        end

        if (npu_status_word !== 32'h4E00_8008 || npu_logit0 !== 32'h0000_0014 ||
            npu_logit1 !== 32'h0000_0000 || npu_class_id !== 8'h00) begin
            $display("FAIL: queue payload result mismatch. status=%08x logit0=%08x logit1=%08x class=%02x",
                     npu_status_word, npu_logit0, npu_logit1, npu_class_id);
            $finish;
        end

        $display("PASS: NPU queue payload-path regression completed.");
        $finish;
    end
endmodule
