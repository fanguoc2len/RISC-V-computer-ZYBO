...
set saved_log [file join $build_dir vivado_zybo_cmdq_dispatch_stub.log]

cd $repo_dir
source [file join $script_dir create_vivado_zybo_project.tcl]

catch {close_sim}
set_property top accel_cmdq_dispatch_stub_tb [get_filesets sim_1]
update_compile_order -fileset sim_1

puts "Starting command-queue dispatch stub simulation..."
...
if {[string first "PASS: command-queue dispatch stub regression completed." $contents] < 0} {
    error "PASS marker was not found in $saved_log"
}

puts "Verified command-queue dispatch stub."
puts "  log: $saved_log"
