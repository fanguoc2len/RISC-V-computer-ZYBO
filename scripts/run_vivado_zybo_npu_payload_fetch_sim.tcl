...
set saved_log [file join $build_dir vivado_zybo_npu_payload_fetch.log]

cd $repo_dir
source [file join $script_dir create_vivado_zybo_project.tcl]

catch {close_sim}
set_property top accel_npu_payload_fetch_stub_tb [get_filesets sim_1]
update_compile_order -fileset sim_1

puts "Starting NPU payload fetch stub simulation..."
...
if {[string first "PASS: NPU payload fetch stub regression completed." $contents] < 0} {
    error "PASS marker was not found in $saved_log"
}

puts "Verified NPU payload fetch stub."
puts "  log: $saved_log"
