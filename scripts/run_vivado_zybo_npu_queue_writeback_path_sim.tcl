...
set saved_log [file join $build_dir vivado_zybo_npu_queue_writeback_path.log]

cd $repo_dir
source [file join $script_dir create_vivado_zybo_project.tcl]

catch {close_sim}
set_property top accel_npu_queue_writeback_path_tb [get_filesets sim_1]
update_compile_order -fileset sim_1

puts "Starting NPU queue writeback-path simulation..."
...
if {[string first "PASS: NPU queue writeback-path regression completed." $contents] < 0} {
    error "PASS marker was not found in $saved_log"
}

puts "Verified NPU queue writeback-path regression."
puts "  log: $saved_log"
